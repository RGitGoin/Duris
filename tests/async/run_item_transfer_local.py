#!/usr/bin/env python3
"""Run the native item transfer owner in an explicitly disposable local schema."""
import os
...
chunk = script.split("g++ -std=c++20", 1)[1].split('"$ROOT/bin/tests/item_transfer_mysql_harness"', 1)[0]
files = re.findall(r"(?:tests|src)/[A-Za-z0-9_/.-]+\.(?:cpp|c)", chunk)
assert files[0] == "tests/async/item_transfer_mysql_harness.cpp"
with tempfile.TemporaryDirectory(prefix="duris-item-owner-") as directory:
    executable = Path(directory) / "item"
    ...
    subprocess.run([str(executable)], cwd=ROOT, check=True,
                   env=dict(os.environ, ITEM_TRANSFER_TEST_DB_NAME=os.environ["DB_NAME"],
                            ASAN_OPTIONS="detect_leaks=1:halt_on_error=1",
                            UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1"))
print("Native item transfer owner qualification passed", flush=True)
