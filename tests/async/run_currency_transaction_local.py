#!/usr/bin/env python3
"""Run the native coin/currency owner in an explicitly disposable local schema."""
import os
...
assert files[0] == "tests/async/currency_transaction_mysql_harness.cpp"
with tempfile.TemporaryDirectory(prefix="duris-currency-owner-") as directory:
    executable = Path(directory) / "currency"
    ...
    subprocess.run([str(executable)], cwd=ROOT, check=True,
                   env=dict(os.environ, CURRENCY_TEST_DB_NAME=os.environ["DB_NAME"],
                            ASAN_OPTIONS="detect_leaks=1:halt_on_error=1",
                            UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1"))
print("Native currency/coin owner qualification passed", flush=True)
